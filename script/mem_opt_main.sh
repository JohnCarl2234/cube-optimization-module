#!/system/bin/sh
#Cube Memory Management Optimization
#Script by: John Carl Acosta
...
config_zram()
{
    ...
    case "$zram_size" in
        0.0|0|0.5|1.0|1|1.7|2.0|2|2.5|3.0|3|4.0|4|5.0|5|6.0|6) ;;
    ...
    case "$zram_size" in
        ...
        1.7)    mem_start_zram 1792M 640M "$zram_alg" ;;
...
save_panel()
{
    clear_panel
    write_panel ""
    write_panel "# Cube Memory Management Optimization"
    write_panel "# https://github.com/JohnCarl2234/cube-optimization-module"
    write_panel "# Created by: rei_ryota"
    write_panel "# Cube Version: 0.1.000 (initial release)"
    #if you want to make your own version please change the status to Unofficial#
    write_panel "# Cube Status: Official"
    ...
    write_panel "# Available size(GB): 0 / 0.5 / 1 / 1.5 / 2 / 2.5 / 3 / 4"
...
# disable Exynos per process reclaim for low-tier or mid-tier devices
lock_val "0" /sys/module/process_reclaim/parameters/enable_process_reclaim

# Exynos Chipsets need more time
sleep 10
...
[ "$(cat $VM/swappiness)" -le 160 ] && lock_val "160" $VM/swappiness
